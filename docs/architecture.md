...
- **New decoration kind** (e.g., syntax highlighting on code blocks): extend `SpanKind`, have the relevant chunker emit spans with that kind, add a visual to `spanVisuals` in `app.tsx` — that `Record<SpanKind, SpanVisual>` _is_ the decorator registry, and being exhaustive over `SpanKind` it won't compile until the new kind has one. Cover it in `chunker.test.ts` by asserting the new spans are dropped from `computeTypeableIndices` — the cosmetic/typeable invariant that file already guards, so a newcomer can't ship spans the typist would have to type through.
