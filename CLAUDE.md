...
- **Diff-aware rendering (optional layer):** with a `--diff` flag (or auto-detected), `+`/`-` lines render differently *without* the core engine knowing what a diff is. This is the test that the *layerable rendering* goal actually holds.
