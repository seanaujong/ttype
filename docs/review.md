...
- outcome: `correct` / `wrong` / `auto-missed` (the auto-marked-red chars from the _enter mid-line_ scenario) / `correction` (a backspace that cleared a wrong char) / `revisit` (a backspace that cleared a correct char)
...
This matches the _general-purpose engine_ goal in CLAUDE.md and lets us ship the engine + a minimal review, then evolve review independently.
...
The premise: typing a piece of text engages with it more deeply than reading. Review helps you notice _what_ you engaged with.
...
## What review explicitly is _not_ (in v1)
...
- **Do we show _anything_ during the run** — a timer, a progress bar — or is the screen purely text + cursor? Lean: nothing during the run. Review is the surface for measurement.
